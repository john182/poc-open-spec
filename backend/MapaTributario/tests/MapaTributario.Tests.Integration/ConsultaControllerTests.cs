...
public class ConsultaControllerTests : IntegrationTestBase
{
    ...
    private async Task SeedTestDataAsync()
    {
        ...
        var execucoes = Database.GetCollection<ExecucaoCrawler>("execucoesCrawler");
        var execucao = ExecucaoCrawler.Create(TipoExecucao.Agendado);
        execucao.SetUfsProcessadas(new[] { "SP", "RJ" });
        execucao.Finalizar(StatusExecucao.Concluido);
        await execucoes.InsertOneAsync(execucao);
    }
    ...
    public async Task GET_MunicipiosPorUf_UfValida_Retorna200()
    {
        ...
        response.StatusCode.ShouldBe(HttpStatusCode.OK);
        var resultado = await response.Content.ReadFromJsonAsync<MunicipiosUfResponse>();
        resultado.ShouldNotBeNull();
        resultado.StatusProcessamento.ShouldBe("concluido");
        resultado.UltimoProcessamento.ShouldNotBeNull();
        resultado.Municipios.Count.ShouldBeGreaterThan(0);
        resultado.Municipios.All(m => m.SiglaEstado == "SP").ShouldBeTrue();
    }
    ...
    public async Task GET_AliquotasPorMunicipio_FiltroAliquotaMinMax_Funciona()
    {
        ...
        paginado.ShouldNotBeNull();
        paginado.Items.Count.ShouldBe(1);
        paginado.Items[0].Aliquota.ShouldBe(3.0m);
    }
    ...
    public async Task GET_DetalheAliquota_Existente_Retorna200()
    {
        ...
        detalhe.Aliquota.ShouldBe(2.0m);
